...
export const projects = sqliteTable(
  "projects",
  {
    ...
    field: text("field").notNull().default("combinatorics"),
    statement: text("statement").notNull(),
    definitionsJson: text("definitions_json").notNull().default("[]"),
    assumptionsJson: text("assumptions_json").notNull().default("[]"),
    knownResultsJson: text("known_results_json").notNull().default("[]"),
    boundsJson: text("bounds_json").notNull().default("{}"),
    mode: text("mode").notNull().default("prove"),
    existingProof: text("existing_proof"),
    userSourcesJson: text("user_sources_json").notNull().default("[]"),
...
export const runProfiles = sqliteTable(
  "run_profiles",
  {
    id: text("id").primaryKey(),
    ownerId: text("owner_id").notNull(),
...
export const researchRuns = sqliteTable(
  ...
  (table) => [
    index("idx_runs_owner_project").on(table.ownerId, table.projectId),
    index("idx_runs_status_updated").on(table.status, table.updatedAt),
  ],
);

export const stageResults = sqliteTable(
  "stage_results",
  {
    id: text("id").primaryKey(),
    runId: text("run_id")
    ...
    toolRecordsJson: text("tool_records_json").notNull().default("[]"),
    errorJson: text("error_json"),
    promptVersion: text("prompt_version").notNull(),
    isDeepPass: integer("is_deep_pass", { mode: "boolean" }).notNull().default(false),
    createdAt: integer("created_at").notNull(),
  },
...
export const idempotencyRecords = sqliteTable(
  ...
  (table) => [
    uniqueIndex("idx_idempotency_owner_key_operation").on(table.ownerId, table.key, table.operation),
  ],
);

export const literatureCache = sqliteTable(
  "literature_cache",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    uniqueIndex("idx_literature_cache_query").on(table.queryKey),
    index("idx_literature_cache_expiry").on(table.expiresAt),
  ],
);
