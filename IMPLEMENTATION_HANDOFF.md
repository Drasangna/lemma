...
> Continue the Lemma private mathematics-research workspace in this repository. First read `IMPLEMENTATION_HANDOFF.md` (including the Milestone 2 addendum below), `HOW_TO_USE.md`, `lib/research-types.ts`, `lib/providers/types.ts`, `lib/orchestrator.ts`, and `db/schema.ts`. Preserve all invariants in the handoff, especially explicit model selection, no automatic paid retry or fallback, application-owned stage schemas, owner checks, idempotent mutations, bounded external tools, and the five claim statuses. Inspect the current worktree before editing and do not undo existing work. Run mocked tests before any opt-in live smoke test. Do not consume model credits unless I explicitly authorize the live call and its model/output cap. This is a local single-user app — there is no hosted Site and no sign-in step to preserve.

## Milestone 2 addendum: local-only, OpenRouter, renamed to Lemma

Everything above this section describes milestone one as originally built and is preserved as historical record. A second milestone then made three deliberate architectural changes, on the owner's explicit instruction:

1. **Renamed the product from Axiom to Lemma.** The old name claimed more than the app ever asserts — nothing here is ever labeled `proved`. Every reference to Axiom in code, docs, and branding was replaced; `PROMPT_VERSION`/algorithm-version strings changed from `axiom-*` to `lemma-*`, which affects provenance strings recorded on new stage results but not on already-persisted rows.
2. **Replaced the direct OpenAI and DeepSeek adapters with a single OpenRouter adapter.** Both original adapters called the OpenAI *Responses API* shape; OpenRouter only speaks *Chat Completions*, so this was a genuine rewrite (`lib/providers/openrouter.ts`), not a URL swap. `ProviderId` was dropped from the schema entirely — a model is now just a free-text OpenRouter-prefixed slug (e.g. `deepseek/deepseek-chat`), and the vendor is implicit in that string. Set only `OPENROUTER_API_KEY`; `OPENAI_API_KEY`/`DEEPSEEK_API_KEY` are no longer read anywhere. The "never automatic retry or fallback across models" invariant is unchanged.
