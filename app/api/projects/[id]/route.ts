...
export async function PATCH(request:Request,{params}:Context){try{const user=await requireApiUser();const {id}=await params;await getOwnedProject(user.userId,id);const spec=problemSpecSchema.parse(await request.json());const [row]=await getDb().update(projects).set({title:spec.title,field:spec.field,statement:spec.statement,definitionsJson:JSON.stringify(spec.definitions),assumptionsJson:JSON.stringify(spec.assumptions),knownResultsJson:JSON.stringify(spec.knownResults),boundsJson:spec.bounds?JSON.stringify(spec.bounds):"{}",mode:spec.mode,existingProof:spec.existingProof??null,userSourcesJson:JSON.stringify(spec.userSources),updatedAt:Date.now()}).where(and(eq(projects.id,id),eq(projects.ownerId,user.userId))).returning();return Response.json({project:serializeProject(row)})}catch(error){return errorResponse(error)}}
