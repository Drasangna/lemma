...
export async function searchLiterature(query:string,mailto?:string,limit=12):Promise<LiteratureSource[]>{
  ...
  const [crossref,arxiv]=await Promise.allSettled([fetch(crossrefUrl,{headers:{"User-Agent":`AxiomResearch/1.0 (${mailto??"contact unavailable"})`}}),fetch(arxivUrl,{headers:{"User-Agent":"AxiomResearch/1.0"}})]);
