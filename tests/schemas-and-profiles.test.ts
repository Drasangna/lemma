...
test("built-in profiles are explicit and capped",()=>{assert.equal(BUILT_IN_PROFILES["deepseek-economy"].stages.proofs.model,"deepseek/deepseek-chat");assert.equal(BUILT_IN_PROFILES["mixed-economy"].stages.lemmas.model,"openai/gpt-4o-mini");for(const profile of Object.values(BUILT_IN_PROFILES))assert.ok(profile.totalOutputLimit<=18_000)});
